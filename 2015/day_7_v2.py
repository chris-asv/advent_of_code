...
# Transforming the input in separable items
...
# Here I will save values
wires = {}

# Defin de function to use
def find_signal(wire):
    try:
        # If wire has been already defined IN LINES_
        return int(wire)
    except:
        if wire not in wires.keys():
            ...
            # If there are two elements, always is a 'NOT' binary operation:
                ...
                wires[wire] = bit_op["NOT"](find_signal(lines_[wire][1]))
            # If there are three elements, just use the function an inputs
            elif len(lines_[wire]) == 3:
                wires[wire] = bit_op[lines_[wire][1]](find_signal(lines_[wire][0]), 
...
# Onyl save 'b' with the value of 'a'
wires = {'b' : wires['a']}
